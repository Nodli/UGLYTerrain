...
TEST_CASE("Test SimpleLayerMap functionment", "[SimpleLayerMap]")
{
	SimpleLayerMap sf(2, 2);
	sf.set_value(0, 0, 0);
...
TEST_CASE("Test SimpleLayerMap equality", "[SimpleLayerMap]")
{
	SimpleLayerMap sf(2, 2);
	sf.set_value(0, 0, 0);
	sf.set_value(0, 1, 0.5);
	...
	{
		SimpleLayerMap sf2(2, 2, {0, 0}, {2, 2});
		sf2.copy_values(sf);
	...
	{
		SimpleLayerMap sf2(5, 5, {0, 0}, {2, 2});
		REQUIRE_THROWS(sf2.copy_values(sf));
	}
}

TEST_CASE("Test SimpleLayerMap addition", "[SimpleLayerMap]")
{
	SimpleLayerMap sf1(2, 2);
	sf1.set_value(0, 0, 0);
	sf1.set_value(0, 1, 0.25);
	sf1.set_value(1, 0, 0.5);
	sf1.set_value(1, 1, 1.0);

	SimpleLayerMap sf2(2, 2);
...
TEST_CASE("Test if Obj exporter", "[SimpleLayerMap]")
{
	SimpleLayerMap sf(2, 2);
	sf.set_value(0, 0, 0);
	sf.set_value(0, 1, 0.5);
...
{
	SimpleLayerMap sf(100, 100, { -25, -25}, {25, 25});
	...
	SimpleLayerMap::generate_slope_map(sf).export_as_pgm("Test_pgm_slope.pgm");
	SimpleLayerMap::generate_slope_map(sf).export_as_obj("Test_pgm_slope.obj");
}